...
contract('geniusToken', function(accounts) {
    ...
    it('initializes the contract with the correct values', function(){
        return geniusToken.deployed().then(function(instance){
            ...
            assert.equal(name, 'Genius Token', 'has the correct name')
            return tokenInstance.symbol();
        }).then(function(symbol) {
            assert.equal(symbol, 'GNS', 'has correct symbol');
            return tokenInstance.standard()
        }).then(function(standard){
            assert.equal(standard, 'Genius Token v1.0', 'has correct standard')
        })
    })
    ...
    it('transfers token ownership', function() {
        return geniusToken.deployed().then(function(instance) {
            tokenInstance = instance;
            // Test require statement first by transferring some large amount
            ...
            assert.equal(receipt.logs[0].args._value, 250000, 'logs the value transferred')
